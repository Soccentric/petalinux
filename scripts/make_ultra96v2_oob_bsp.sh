...
petalinux_project_set_sstate_paths() {
  ...
  if [ -d ${PETALINUX}/${CACHE_DOWNLOADS} ]; then
    echo " "
  ...
  if [ -d ${PETALINUX}/${CACHE_AARCH64} ]; then
    echo " "
...
create_petalinux_bsp() {
  ...
  if [ -f ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/project/config.${PETALINUX_ROOTFS_NAME}.patch ]; then
    echo " "
    ...
    cd ${START_FOLDER}/${PETALINUX_PROJECTS_FOLDER}/${PETALINUX_PROJECT_NAME}
  elif [ -f ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/project/config.${PETALINUX_ROOTFS_NAME} ]; then
    echo " "
  ...
  elif [ -f ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/project/config.generic ]; then
    echo " "
  ...
  # Copy the rootfs to the pre-built images folder.  This is needed for
  # systems that boot the rootfs from a ext4 partition on the micro SD card.
  cp ${START_FOLDER}/${PETALINUX_PROJECTS_FOLDER}/${PETALINUX_PROJECT_NAME}/images/linux/rootfs.tar.gz \
    ${START_FOLDER}/${PETALINUX_PROJECTS_FOLDER}/${PETALINUX_PROJECT_NAME}/pre-built/linux/images/.
...
build_hw_platform() {
  ...
  if [ ! -e ${HDL_PROJECT_NAME}/${HDL_BOARD_NAME}_${PLNX_VER}/${HDL_BOARD_NAME}.dsa ]; then
